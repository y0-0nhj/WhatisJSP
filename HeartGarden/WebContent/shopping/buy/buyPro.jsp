...
cartList = cartPro.getCartList(buyer, cart_id);
for(CartDataBean c : cartList) {
	buy = new BuyDataBean();
	buy.setBuy_id(buy_id);
	buy.setBuyer(c.getBuyer());
	buy.setProduct_id(c.getProduct_id());
	buy.setProduct_name(c.getProduct_name());
	buy.setProduct_image(c.getProduct_image());
	buy.setBuy_price(c.getBuy_price());
	buy.setBuy_count(c.getBuy_count());
	buy.setBuy_date(now);
...
int check =  buyPro.insertBuyList(buyList, cartList);
