...
public class BoardQnaDBBean {
	...
	public void insertArticle(BoardQnaDataBean article) {
		...
		try {
			...
			sql = "select max(num) from board_qna";
			...
			// 데이터 추가
			sql = "insert into board_qna(writer, passwd, q_kind, subject, content, reg_date, ref, re_step, re_level) "
					+ "values(?,?,?,?,?,?,?,?,?)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, article.getWriter());
			pstmt.setString(2, article.getPasswd());
			pstmt.setString(3, article.getQ_kind());
	...
	public BoardQnaDataBean getArticle(int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql = "update board_qna set readcount=readcount+1 where num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeLargeUpdate();
			
			sql = "select * from board_qna where num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public BoardQnaDataBean updateGetArticle(int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		BoardQnaDataBean article = null;
	...
	public int updateArticle(BoardQnaDataBean article) {
		...
		try {
			...
			sql = "select passwd from board_qna where num=?";
			pstmt = conn.prepareStatement(sql);
			...
			if(rs.next()) { // 아이디가 있을 때
				...
				if(dbPasswd.equals(article.getPasswd())) { // 비밀번호가 같을 때
					sql = "update board_qna set q_kind=?, subject=?, content=? where num=?";
					pstmt = conn.prepareStatement(sql);
		...
		try {
			...
			if(rs.next()) { // 아이디가 있을 때
				...
				if(dbPasswd.equals(passwd)) { // 비밀번호가 같을 때
					sql = "delete from board_qna where num=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, num);
					pstmt.executeUpdate();
					x = 1;
